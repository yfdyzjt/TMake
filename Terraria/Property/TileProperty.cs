namespace TMake.Terraria
{
    public enum WireColor : byte
    {
        Red = 0x0,
        Blue = 0x1,
        Green = 0x2,
        Yellow = 0x3,
    }
    public enum BrickStyle : byte
    {
        Full = 0x0,
        Half = 0x1,
        SlopeTopRight = 0x2,
        SlopeTopLeft = 0x3,
        SlopeBottomRight = 0x4,
        SlopeBottomLeft = 0x5,
    }
    public enum LiquidType : byte
    {
        None = 0x0,
        Water = 0x01,
        Lava = 0x02,
        Honey = 0x03,
        Shimmer = 0x04,
    }
    public enum TileID : ushort
    {
        Dirt = 0,
        Stone = 1,
        Grass = 2,
        Plants = 3,
        Torches = 4,
        Trees = 5,
        Iron = 6,
        Copper = 7,
        Gold = 8,
        Silver = 9,
        ClosedDoor = 10,
        OpenDoor = 11,
        Heart = 12,
        Bottles = 13,
        Tables = 14,
        Chairs = 15,
        Anvils = 16,
        Furnaces = 17,
        WorkBenches = 18,
        Platforms = 19,
        Saplings = 20,
        Containers = 21,
        Demonite = 22,
        CorruptGrass = 23,
        CorruptPlants = 24,
        Ebonstone = 25,
        DemonAltar = 26,
        Sunflower = 27,
        Pots = 28,
        PiggyBank = 29,
        WoodBlock = 30,
        ShadowOrbs = 31,
        CorruptThorns = 32,
        Candles = 33,
        Chandeliers = 34,
        Jackolanterns = 35,
        Presents = 36,
        Meteorite = 37,
        GrayBrick = 38,
        RedBrick = 39,
        ClayBlock = 40,
        BlueDungeonBrick = 41,
        HangingLanterns = 42,
        GreenDungeonBrick = 43,
        PinkDungeonBrick = 44,
        GoldBrick = 45,
        SilverBrick = 46,
        CopperBrick = 47,
        Spikes = 48,
        WaterCandle = 49,
        Books = 50,
        Cobweb = 51,
        Vines = 52,
        Sand = 53,
        Glass = 54,
        Signs = 55,
        Obsidian = 56,
        Ash = 57,
        Hellstone = 58,
        Mud = 59,
        JungleGrass = 60,
        JunglePlants = 61,
        JungleVines = 62,
        Sapphire = 63,
        Ruby = 64,
        Emerald = 65,
        Topaz = 66,
        Amethyst = 67,
        Diamond = 68,
        JungleThorns = 69,
        MushroomGrass = 70,
        MushroomPlants = 71,
        MushroomTrees = 72,
        Plants2 = 73,
        JunglePlants2 = 74,
        ObsidianBrick = 75,
        HellstoneBrick = 76,
        Hellforge = 77,
        ClayPot = 78,
        Beds = 79,
        Cactus = 80,
        Coral = 81,
        ImmatureHerbs = 82,
        MatureHerbs = 83,
        BloomingHerbs = 84,
        Tombstones = 85,
        Loom = 86,
        Pianos = 87,
        Dressers = 88,
        Benches = 89,
        Bathtubs = 90,
        Banners = 91,
        Lampposts = 92,
        Lamps = 93,
        Kegs = 94,
        ChineseLanterns = 95,
        CookingPots = 96,
        Safes = 97,
        SkullLanterns = 98,
        TrashCan = 99,
        Candelabras = 100,
        Bookcases = 101,
        Thrones = 102,
        Bowls = 103,
        GrandfatherClocks = 104,
        Statues = 105,
        Sawmill = 106,
        Cobalt = 107,
        Mythril = 108,
        HallowedGrass = 109,
        HallowedPlants = 110,
        Adamantite = 111,
        Ebonsand = 112,
        HallowedPlants2 = 113,
        TinkerersWorkbench = 114,
        HallowedVines = 115,
        Pearlsand = 116,
        Pearlstone = 117,
        PearlstoneBrick = 118,
        IridescentBrick = 119,
        Mudstone = 120,
        CobaltBrick = 121,
        MythrilBrick = 122,
        Silt = 123,
        WoodenBeam = 124,
        CrystalBall = 125,
        DiscoBall = 126,
        MagicalIceBlock = 127,
        Mannequin = 128,
        Crystals = 129,
        ActiveStoneBlock = 130,
        InactiveStoneBlock = 131,
        Lever = 132,
        AdamantiteForge = 133,
        MythrilAnvil = 134,
        PressurePlates = 135,
        Switches = 136,
        Traps = 137,
        Boulder = 138,
        MusicBoxes = 139,
        DemoniteBrick = 140,
        Explosives = 141,
        InletPump = 142,
        OutletPump = 143,
        Timers = 144,
        CandyCaneBlock = 145,
        GreenCandyCaneBlock = 146,
        SnowBlock = 147,
        SnowBrick = 148,
        HolidayLights = 149,
        AdamantiteBeam = 150,
        SandstoneBrick = 151,
        EbonstoneBrick = 152,
        RedStucco = 153,
        YellowStucco = 154,
        GreenStucco = 155,
        GrayStucco = 156,
        Ebonwood = 157,
        RichMahogany = 158,
        Pearlwood = 159,
        RainbowBrick = 160,
        IceBlock = 161,
        BreakableIce = 162,
        CorruptIce = 163,
        HallowedIce = 164,
        Stalactite = 165,
        Tin = 166,
        Lead = 167,
        Tungsten = 168,
        Platinum = 169,
        PineTree = 170,
        ChristmasTree = 171,
        Sinks = 172,
        PlatinumCandelabra = 173,
        PlatinumCandle = 174,
        TinBrick = 175,
        TungstenBrick = 176,
        PlatinumBrick = 177,
        ExposedGems = 178,
        GreenMoss = 179,
        BrownMoss = 180,
        RedMoss = 181,
        BlueMoss = 182,
        PurpleMoss = 183,
        LongMoss = 184,
        SmallPiles = 185,
        LargePiles = 186,
        LargePiles2 = 187,
        CactusBlock = 188,
        Cloud = 189,
        MushroomBlock = 190,
        LivingWood = 191,
        LeafBlock = 192,
        SlimeBlock = 193,
        BoneBlock = 194,
        FleshBlock = 195,
        RainCloud = 196,
        FrozenSlimeBlock = 197,
        Asphalt = 198,
        CrimsonGrass = 199,
        FleshIce = 200,
        CrimsonPlants = 201,
        Sunplate = 202,
        Crimstone = 203,
        Crimtane = 204,
        CrimsonVines = 205,
        IceBrick = 206,
        WaterFountain = 207,
        Shadewood = 208,
        Cannon = 209,
        LandMine = 210,
        Chlorophyte = 211,
        SnowballLauncher = 212,
        Rope = 213,
        Chain = 214,
        Campfire = 215,
        Firework = 216,
        Blendomatic = 217,
        MeatGrinder = 218,
        Extractinator = 219,
        Solidifier = 220,
        Palladium = 221,
        Orichalcum = 222,
        Titanium = 223,
        Slush = 224,
        Hive = 225,
        LihzahrdBrick = 226,
        DyePlants = 227,
        DyeVat = 228,
        HoneyBlock = 229,
        CrispyHoneyBlock = 230,
        Larva = 231,
        WoodenSpikes = 232,
        PlantDetritus = 233,
        Crimsand = 234,
        Teleporter = 235,
        LifeFruit = 236,
        LihzahrdAltar = 237,
        PlanteraBulb = 238,
        MetalBars = 239,
        Painting3X3 = 240,
        Painting4X3 = 241,
        Painting6X4 = 242,
        ImbuingStation = 243,
        BubbleMachine = 244,
        Painting2X3 = 245,
        Painting3X2 = 246,
        Autohammer = 247,
        PalladiumColumn = 248,
        BubblegumBlock = 249,
        Titanstone = 250,
        PumpkinBlock = 251,
        HayBlock = 252,
        SpookyWood = 253,
        Pumpkins = 254,
        AmethystGemsparkOff = 255,
        TopazGemsparkOff = 256,
        SapphireGemsparkOff = 257,
        EmeraldGemsparkOff = 258,
        RubyGemsparkOff = 259,
        DiamondGemsparkOff = 260,
        AmberGemsparkOff = 261,
        AmethystGemspark = 262,
        TopazGemspark = 263,
        SapphireGemspark = 264,
        EmeraldGemspark = 265,
        RubyGemspark = 266,
        DiamondGemspark = 267,
        AmberGemspark = 268,
        Womannequin = 269,
        FireflyinaBottle = 270,
        LightningBuginaBottle = 271,
        Cog = 272,
        StoneSlab = 273,
        SandStoneSlab = 274,
        BunnyCage = 275,
        SquirrelCage = 276,
        MallardDuckCage = 277,
        DuckCage = 278,
        BirdCage = 279,
        BlueJay = 280,
        CardinalCage = 281,
        FishBowl = 282,
        HeavyWorkBench = 283,
        CopperPlating = 284,
        SnailCage = 285,
        GlowingSnailCage = 286,
        AmmoBox = 287,
        MonarchButterflyJar = 288,
        PurpleEmperorButterflyJar = 289,
        RedAdmiralButterflyJar = 290,
        UlyssesButterflyJar = 291,
        SulphurButterflyJar = 292,
        TreeNymphButterflyJar = 293,
        ZebraSwallowtailButterflyJar = 294,
        JuliaButterflyJar = 295,
        ScorpionCage = 296,
        BlackScorpionCage = 297,
        FrogCage = 298,
        MouseCage = 299,
        BoneWelder = 300,
        FleshCloningVat = 301,
        GlassKiln = 302,
        LihzahrdFurnace = 303,
        LivingLoom = 304,
        SkyMill = 305,
        IceMachine = 306,
        SteampunkBoiler = 307,
        HoneyDispenser = 308,
        PenguinCage = 309,
        WormCage = 310,
        DynastyWood = 311,
        RedDynastyShingles = 312,
        BlueDynastyShingles = 313,
        MinecartTrack = 314,
        Coralstone = 315,
        BlueJellyfishBowl = 316,
        GreenJellyfishBowl = 317,
        PinkJellyfishBowl = 318,
        ShipInABottle = 319,
        SeaweedPlanter = 320,
        BorealWood = 321,
        PalmWood = 322,
        PalmTree = 323,
        BeachPiles = 324,
        TinPlating = 325,
        Waterfall = 326,
        Lavafall = 327,
        Confetti = 328,
        ConfettiBlack = 329,
        CopperCoinPile = 330,
        SilverCoinPile = 331,
        GoldCoinPile = 332,
        PlatinumCoinPile = 333,
        WeaponsRack = 334,
        FireworksBox = 335,
        LivingFire = 336,
        AlphabetStatues = 337,
        FireworkFountain = 338,
        GrasshopperCage = 339,
        LivingCursedFire = 340,
        LivingDemonFire = 341,
        LivingFrostFire = 342,
        LivingIchor = 343,
        LivingUltrabrightFire = 344,
        Honeyfall = 345,
        ChlorophyteBrick = 346,
        CrimtaneBrick = 347,
        ShroomitePlating = 348,
        MushroomStatue = 349,
        MartianConduitPlating = 350,
        ChimneySmoke = 351,
        CrimsonThorns = 352,
        VineRope = 353,
        BewitchingTable = 354,
        AlchemyTable = 355,
        Sundial = 356,
        MarbleBlock = 357,
        GoldBirdCage = 358,
        GoldBunnyCage = 359,
        GoldButterflyCage = 360,
        GoldFrogCage = 361,
        GoldGrasshopperCage = 362,
        GoldMouseCage = 363,
        GoldWormCage = 364,
        SilkRope = 365,
        WebRope = 366,
        Marble = 367,
        Granite = 368,
        GraniteBlock = 369,
        MeteoriteBrick = 370,
        PinkSlimeBlock = 371,
        PeaceCandle = 372,
        WaterDrip = 373,
        LavaDrip = 374,
        HoneyDrip = 375,
        FishingCrate = 376,
        SharpeningStation = 377,
        TargetDummy = 378,
        Bubble = 379,
        PlanterBox = 380,
        LavaMoss = 381,
        VineFlowers = 382,
        LivingMahogany = 383,
        LivingMahoganyLeaves = 384,
        CrystalBlock = 385,
        TrapdoorOpen = 386,
        TrapdoorClosed = 387,
        TallGateClosed = 388,
        TallGateOpen = 389,
        LavaLamp = 390,
        CageEnchantedNightcrawler = 391,
        CageBuggy = 392,
        CageGrubby = 393,
        CageSluggy = 394,
        ItemFrame = 395,
        Sandstone = 396,
        HardenedSand = 397,
        CorruptHardenedSand = 398,
        CrimsonHardenedSand = 399,
        CorruptSandstone = 400,
        CrimsonSandstone = 401,
        HallowHardenedSand = 402,
        HallowSandstone = 403,
        DesertFossil = 404,
        Fireplace = 405,
        Chimney = 406,
        FossilOre = 407,
        LunarOre = 408,
        LunarBrick = 409,
        LunarMonolith = 410,
        Detonator = 411,
        LunarCraftingStation = 412,
        SquirrelOrangeCage = 413,
        SquirrelGoldCage = 414,
        LunarBlockSolar = 415,
        LunarBlockVortex = 416,
        LunarBlockNebula = 417,
        LunarBlockStardust = 418,
        LogicGateLamp = 419,
        LogicGate = 420,
        ConveyorBeltLeft = 421,
        ConveyorBeltRight = 422,
        LogicSensor = 423,
        WirePipe = 424,
        AnnouncementBox = 425,
        TeamBlockRed = 426,
        TeamBlockRedPlatform = 427,
        WeightedPressurePlate = 428,
        WireBulb = 429,
        TeamBlockGreen = 430,
        TeamBlockBlue = 431,
        TeamBlockYellow = 432,
        TeamBlockPink = 433,
        TeamBlockWhite = 434,
        TeamBlockGreenPlatform = 435,
        TeamBlockBluePlatform = 436,
        TeamBlockYellowPlatform = 437,
        TeamBlockPinkPlatform = 438,
        TeamBlockWhitePlatform = 439,
        GemLocks = 440,
        FakeContainers = 441,
        ProjectilePressurePad = 442,
        GeyserTrap = 443,
        BeeHive = 444,
        PixelBox = 445,
        SillyBalloonPink = 446,
        SillyBalloonPurple = 447,
        SillyBalloonGreen = 448,
        SillyStreamerBlue = 449,
        SillyStreamerGreen = 450,
        SillyStreamerPink = 451,
        SillyBalloonMachine = 452,
        SillyBalloonTile = 453,
        Pigronata = 454,
        PartyMonolith = 455,
        PartyBundleOfBalloonTile = 456,
        PartyPresent = 457,
        SandFallBlock = 458,
        SnowFallBlock = 459,
        SnowCloud = 460,
        SandDrip = 461,
        DjinnLamp = 462,
        DefendersForge = 463,
        WarTable = 464,
        WarTableBanner = 465,
        ElderCrystalStand = 466,
        Containers2 = 467,
        FakeContainers2 = 468,
        Tables2 = 469,
        DisplayDoll = 470,
        WeaponsRack2 = 471,
        IronBrick = 472,
        LeadBrick = 473,
        LesionBlock = 474,
        HatRack = 475,
        GolfHole = 476,
        GolfGrass = 477,
        CrimstoneBrick = 478,
        SmoothSandstone = 479,
        BloodMoonMonolith = 480,
        CrackedBlueDungeonBrick = 481,
        CrackedGreenDungeonBrick = 482,
        CrackedPinkDungeonBrick = 483,
        RollingCactus = 484,
        AntlionLarva = 485,
        DrumSet = 486,
        PicnicTable = 487,
        FallenLog = 488,
        PinWheel = 489,
        WeatherVane = 490,
        VoidVault = 491,
        GolfGrassHallowed = 492,
        GolfCupFlag = 493,
        GolfTee = 494,
        ShellPile = 495,
        AntiPortalBlock = 496,
        Toilets = 497,
        Spider = 498,
        LesionStation = 499,
        SolarBrick = 500,
        VortexBrick = 501,
        NebulaBrick = 502,
        StardustBrick = 503,
        MysticSnakeRope = 504,
        GoldGoldfishBowl = 505,
        CatBast = 506,
        GoldStarryGlassBlock = 507,
        BlueStarryGlassBlock = 508,
        VoidMonolith = 509,
        ArrowSign = 510,
        PaintedArrowSign = 511,
        GreenMossBrick = 512,
        BrownMossBrick = 513,
        RedMossBrick = 514,
        BlueMossBrick = 515,
        PurpleMossBrick = 516,
        LavaMossBrick = 517,
        LilyPad = 518,
        Cattail = 519,
        FoodPlatter = 520,
        BlackDragonflyJar = 521,
        BlueDragonflyJar = 522,
        GreenDragonflyJar = 523,
        OrangeDragonflyJar = 524,
        RedDragonflyJar = 525,
        YellowDragonflyJar = 526,
        GoldDragonflyJar = 527,
        MushroomVines = 528,
        SeaOats = 529,
        OasisPlants = 530,
        BoulderStatue = 531,
        MaggotCage = 532,
        RatCage = 533,
        KryptonMoss = 534,
        KryptonMossBrick = 535,
        XenonMoss = 536,
        XenonMossBrick = 537,
        LadybugCage = 538,
        ArgonMoss = 539,
        ArgonMossBrick = 540,
        EchoBlock = 541,
        OwlCage = 542,
        PupfishBowl = 543,
        GoldLadybugCage = 544,
        LawnFlamingo = 545,
        Grate = 546,
        PottedPlants1 = 547,
        PottedPlants2 = 548,
        Seaweed = 549,
        TurtleCage = 550,
        TurtleJungleCage = 551,
        Sandcastles = 552,
        GrebeCage = 553,
        SeagullCage = 554,
        WaterStriderCage = 555,
        GoldWaterStriderCage = 556,
        GrateClosed = 557,
        SeahorseCage = 558,
        GoldSeahorseCage = 559,
        GolfTrophies = 560,
        MarbleColumn = 561,
        BambooBlock = 562,
        LargeBambooBlock = 563,
        PlasmaLamp = 564,
        FogMachine = 565,
        AmberStoneBlock = 566,
        GardenGnome = 567,
        PinkFairyJar = 568,
        GreenFairyJar = 569,
        BlueFairyJar = 570,
        Bamboo = 571,
        SoulBottles = 572,
        TatteredWoodSign = 573,
        BorealBeam = 574,
        RichMahoganyBeam = 575,
        GraniteColumn = 576,
        SandstoneColumn = 577,
        MushroomBeam = 578,
        RockGolemHead = 579,
        HellButterflyJar = 580,
        LavaflyinaBottle = 581,
        MagmaSnailCage = 582,
        TreeTopaz = 583,
        TreeAmethyst = 584,
        TreeSapphire = 585,
        TreeEmerald = 586,
        TreeRuby = 587,
        TreeDiamond = 588,
        TreeAmber = 589,
        GemSaplings = 590,
        PotsSuspended = 591,
        BrazierSuspended = 592,
        VolcanoSmall = 593,
        VolcanoLarge = 594,
        VanityTreeSakuraSaplings = 595,
        VanityTreeSakura = 596,
        TeleportationPylon = 597,
        LavafishBowl = 598,
        AmethystBunnyCage = 599,
        TopazBunnyCage = 600,
        SapphireBunnyCage = 601,
        EmeraldBunnyCage = 602,
        RubyBunnyCage = 603,
        DiamondBunnyCage = 604,
        AmberBunnyCage = 605,
        AmethystSquirrelCage = 606,
        TopazSquirrelCage = 607,
        SapphireSquirrelCage = 608,
        EmeraldSquirrelCage = 609,
        RubySquirrelCage = 610,
        DiamondSquirrelCage = 611,
        AmberSquirrelCage = 612,
        PottedLavaPlants = 613,
        PottedLavaPlantTendrils = 614,
        VanityTreeWillowSaplings = 615,
        VanityTreeYellowWillow = 616,
        MasterTrophyBase = 617,
        AccentSlab = 618,
        TruffleWormCage = 619,
        EmpressButterflyJar = 620,
        SliceOfCake = 621,
        TeaKettle = 622,
        PottedCrystalPlants = 623,
        AbigailsFlower = 624,
        VioletMoss = 625,
        VioletMossBrick = 626,
        RainbowMoss = 627,
        RainbowMossBrick = 628,
        StinkbugCage = 629,
        StinkbugHousingBlocker = 630,
        StinkbugHousingBlockerEcho = 631,
        ScarletMacawCage = 632,
        AshGrass = 633,
        TreeAsh = 634,
        AshWood = 635,
        CorruptVines = 636,
        AshPlants = 637,
        AshVines = 638,
        ManaCrystal = 639,
        BlueMacawCage = 640,
        ReefBlock = 641,
        ChlorophyteExtractinator = 642,
        ToucanCage = 643,
        YellowCockatielCage = 644,
        GrayCockatielCage = 645,
        ShadowCandle = 646,
        LargePilesEcho = 647,
        LargePiles2Echo = 648,
        SmallPiles2x1Echo = 649,
        SmallPiles1x1Echo = 650,
        PlantDetritus3x2Echo = 651,
        PlantDetritus2x2Echo = 652,
        PotsEcho = 653,
        TNTBarrel = 654,
        PlanteraThorns = 655,
        GlowTulip = 656,
        EchoMonolith = 657,
        ShimmerMonolith = 658,
        ShimmerBlock = 659,
        ShimmerflyinaBottle = 660,
        CorruptJungleGrass = 661,
        CrimsonJungleGrass = 662,
        Moondial = 663,
        BouncyBoulder = 664,
        LifeCrystalBoulder = 665,
        PoopBlock = 666,
        ShimmerBrick = 667,
        DirtiestBlock = 668,
        LunarRustBrick = 669,
        DarkCelestialBrick = 670,
        AstraBrick = 671,
        CosmicEmberBrick = 672,
        CryocoreBrick = 673,
        MercuryBrick = 674,
        StarRoyaleBrick = 675,
        HeavenforgeBrick = 676,
        AncientBlueBrick = 677,
        AncientGreenBrick = 678,
        AncientPinkBrick = 679,
        AncientGoldBrick = 680,
        AncientSilverBrick = 681,
        AncientCopperBrick = 682,
        AncientObsidianBrick = 683,
        AncientHellstoneBrick = 684,
        AncientCobaltBrick = 685,
        AncientMythrilBrick = 686,
        LavaMossBlock = 687,
        ArgonMossBlock = 688,
        KryptonMossBlock = 689,
        XenonMossBlock = 690,
        VioletMossBlock = 691,
        RainbowMossBlock = 692,
        Count = 693
    }
    public enum FrameAnchor
    {
        None,
        Left,
        Right,
        Top,
        Bottom
    }
    public static class TileProperty
    {
        public static TileData GetTileData(ushort type)
        {
            ushort usetype = (type > TileDatas.Keys.Max()) ? TileDatas.Keys.Max() : type;
            if (TileDatas.TryGetValue(usetype, out var data))
            {
                return data;
            }
            else
            {
                throw new ArgumentOutOfRangeException(nameof(type), $"Missing tile data: {type}");
            }
        }
        public readonly static Dictionary<ushort, TileData> TileDatas = new()
        {
            [0] = new TileData()
            {
                Type = 0,
                Name = "Dirt_Block",
                Solid = true,
            },
            [1] = new TileData()
            {
                Type = 1,
                Name = "Stone_Block",
                Solid = true,
            },
            [2] = new TileData()
            {
                Type = 2,
                Name = "Grass_Block",
                Solid = true,
            },
            [3] = new TileData()
            {
                Type = 3,
                Name = "Forest_Short_Plants",
                TextureGrid = new(16, 20),
            },
            [4] = new TileData()
            {
                Type = 4,
                Name = "Torches",
                TextureGrid = new(20, 20),
            },
            [5] = new TileData()
            {
                Type = 5,
                Name = "Trees",
                TextureGrid = new(20, 20),
            },
            [6] = new TileData()
            {
                Type = 6,
                Name = "Iron_Ore",
                Solid = true,
            },
            [7] = new TileData()
            {
                Type = 7,
                Name = "Copper_Ore",
                Solid = true,
            },
            [8] = new TileData()
            {
                Type = 8,
                Name = "Gold_Ore",
                Solid = true,
            },
            [9] = new TileData()
            {
                Type = 9,
                Name = "Silver_Ore",
                Solid = true,
            },
            [10] = new TileData()
            {
                Type = 10,
                Name = "Doors_(Closed)",
                Solid = true,
            },
            [11] = new TileData()
            {
                Type = 11,
                Name = "Doors_(Open)",
            },
            [12] = new TileData()
            {
                Type = 12,
                Name = "Crystal_Heart",
            },
            [13] = new TileData()
            {
                Type = 13,
                Name = "Placed_Bottles",
            },
            [14] = new TileData()
            {
                Type = 14,
                Name = "Tables",
            },
            [15] = new TileData()
            {
                Type = 15,
                Name = "Chairs",
            },
            [16] = new TileData()
            {
                Type = 16,
                Name = "Anvils",
                TextureGrid = new(16, 18),
            },
            [17] = new TileData()
            {
                Type = 17,
                Name = "Furnace",
            },
            [18] = new TileData()
            {
                Type = 18,
                Name = "Work_Benches",
                TextureGrid = new(16, 18),
            },
            [19] = new TileData()
            {
                Type = 19,
                Name = "Platforms",
                Solid = true,
            },
            [20] = new TileData()
            {
                Type = 20,
                Name = "Saplings",
            },
            [21] = new TileData()
            {
                Type = 21,
                Name = "Chests",
            },
            [22] = new TileData()
            {
                Type = 22,
                Name = "Demonite_Ore",
                Solid = true,
            },
            [23] = new TileData()
            {
                Type = 23,
                Name = "Corrupt_Grass_Block",
                Solid = true,
            },
            [24] = new TileData()
            {
                Type = 24,
                Name = "Corruption_Short_Plants",
                TextureGrid = new(16, 20),
            },
            [25] = new TileData()
            {
                Type = 25,
                Name = "Ebonstone_Block",
                Solid = true,
            },
            [26] = new TileData()
            {
                Type = 26,
                Name = "Altars",
            },
            [27] = new TileData()
            {
                Type = 27,
                Name = "Sunflower",
            },
            [28] = new TileData()
            {
                Type = 28,
                Name = "Pots",
            },
            [29] = new TileData()
            {
                Type = 29,
                Name = "Piggy_Bank",
            },
            [30] = new TileData()
            {
                Type = 30,
                Name = "Wood",
                Solid = true,
            },
            [31] = new TileData()
            {
                Type = 31,
                Name = "Orb_Heart",
            },
            [32] = new TileData()
            {
                Type = 32,
                Name = "Corruption_Thorns",
            },
            [33] = new TileData()
            {
                Type = 33,
                Name = "Candles",
                TextureGrid = new(16, 20),
            },
            [34] = new TileData()
            {
                Type = 34,
                Name = "Chandeliers",
            },
            [35] = new TileData()
            {
                Type = 35,
                Name = "Jack_'O_Lantern",
            },
            [36] = new TileData()
            {
                Type = 36,
                Name = "Presents",
            },
            [37] = new TileData()
            {
                Type = 37,
                Name = "Meteorite_Ore",
                Solid = true,
            },
            [38] = new TileData()
            {
                Type = 38,
                Name = "Gray_Brick",
                Solid = true,
            },
            [39] = new TileData()
            {
                Type = 39,
                Name = "Red_Brick",
                Solid = true,
            },
            [40] = new TileData()
            {
                Type = 40,
                Name = "Clay_Block",
                Solid = true,
            },
            [41] = new TileData()
            {
                Type = 41,
                Name = "Blue_Brick",
                Solid = true,
            },
            [42] = new TileData()
            {
                Type = 42,
                Name = "Lanterns",
            },
            [43] = new TileData()
            {
                Type = 43,
                Name = "Green_Brick",
                Solid = true,
            },
            [44] = new TileData()
            {
                Type = 44,
                Name = "Pink_Brick",
                Solid = true,
            },
            [45] = new TileData()
            {
                Type = 45,
                Name = "Gold_Brick",
                Solid = true,
            },
            [46] = new TileData()
            {
                Type = 46,
                Name = "Silver_Brick",
                Solid = true,
            },
            [47] = new TileData()
            {
                Type = 47,
                Name = "Copper_Brick",
                Solid = true,
            },
            [48] = new TileData()
            {
                Type = 48,
                Name = "Spike",
                Solid = true,
            },
            [49] = new TileData()
            {
                Type = 49,
                Name = "Water_Candle",
                TextureGrid = new(16, 20),
            },
            [50] = new TileData()
            {
                Type = 50,
                Name = "Books",
            },
            [51] = new TileData()
            {
                Type = 51,
                Name = "Cobweb",
                Solid = true,
            },
            [52] = new TileData()
            {
                Type = 52,
                Name = "Vines",
                Solid = true,
            },
            [53] = new TileData()
            {
                Type = 53,
                Name = "Sand_Block",
                Solid = true,
            },
            [54] = new TileData()
            {
                Type = 54,
                Name = "Glass_Block",
                Solid = true,
            },
            [55] = new TileData()
            {
                Type = 55,
                Name = "Sign",
            },
            [56] = new TileData()
            {
                Type = 56,
                Name = "Obsidian_Block",
                Solid = true,
            },
            [57] = new TileData()
            {
                Type = 57,
                Name = "Ash_Block",
                Solid = true,
            },
            [58] = new TileData()
            {
                Type = 58,
                Name = "Hellstone_Ore",
                Solid = true,
            },
            [59] = new TileData()
            {
                Type = 59,
                Name = "Mud_Block",
                Solid = true,
            },
            [60] = new TileData()
            {
                Type = 60,
                Name = "Jungle_Grass_Block",
                Solid = true,
            },
            [61] = new TileData()
            {
                Type = 61,
                Name = "Jungle_Short_Plants",
                TextureGrid = new(16, 20),
            },
            [62] = new TileData()
            {
                Type = 62,
                Name = "Jungle_Vines",
                Solid = true,
            },
            [63] = new TileData()
            {
                Type = 63,
                Name = "Sapphire_Stone_Block",
                Solid = true,
            },
            [64] = new TileData()
            {
                Type = 64,
                Name = "Ruby_Stone_Block",
                Solid = true,
            },
            [65] = new TileData()
            {
                Type = 65,
                Name = "Emerald_Stone_Block",
                Solid = true,
            },
            [66] = new TileData()
            {
                Type = 66,
                Name = "Topaz_Stone_Block",
                Solid = true,
            },
            [67] = new TileData()
            {
                Type = 67,
                Name = "Amethyst_Stone_Block",
                Solid = true,
            },
            [68] = new TileData()
            {
                Type = 68,
                Name = "Diamond_Stone_Block",
                Solid = true,
            },
            [69] = new TileData()
            {
                Type = 69,
                Name = "Jungle_Thorns",
            },
            [70] = new TileData()
            {
                Type = 70,
                Name = "Mushroom_Grass_Block",
                Solid = true,
            },
            [71] = new TileData()
            {
                Type = 71,
                Name = "Mushroom_Plant",
                TextureGrid = new(16, 20),
            },
            [72] = new TileData()
            {
                Type = 72,
                Name = "Giant_Glowing_Mushroom_(Underground)",
            },
            [73] = new TileData()
            {
                Type = 73,
                Name = "Forest_Tall_Plants",
                TextureGrid = new(16, 32),
            },
            [74] = new TileData()
            {
                Type = 74,
                Name = "Jungle_Tall_Plants",
                TextureGrid = new(16, 32),
            },
            [75] = new TileData()
            {
                Type = 75,
                Name = "Obsidian_Brick",
                Solid = true,
            },
            [76] = new TileData()
            {
                Type = 76,
                Name = "Hellstone_Brick",
                Solid = true,
            },
            [77] = new TileData()
            {
                Type = 77,
                Name = "Hellforge",
            },
            [78] = new TileData()
            {
                Type = 78,
                Name = "Clay_Pot",
            },
            [79] = new TileData()
            {
                Type = 79,
                Name = "Beds",
            },
            [80] = new TileData()
            {
                Type = 80,
                Name = "Cactus_Plant",
            },
            [81] = new TileData()
            {
                Type = 81,
                Name = "Coral",
                TextureGrid = new(24, 26),
            },
            [82] = new TileData()
            {
                Type = 82,
                Name = "Herbs_(Sprout)",
            },
            [83] = new TileData()
            {
                Type = 83,
                Name = "Herbs_(Mature)",
            },
            [84] = new TileData()
            {
                Type = 84,
                Name = "Herbs_(Bloom)",
            },
            [85] = new TileData()
            {
                Type = 85,
                Name = "Tombstones",
            },
            [86] = new TileData()
            {
                Type = 86,
                Name = "Loom",
            },
            [87] = new TileData()
            {
                Type = 87,
                Name = "Pianos",
            },
            [88] = new TileData()
            {
                Type = 88,
                Name = "Dressers",
            },
            [89] = new TileData()
            {
                Type = 89,
                Name = "Benches",
            },
            [90] = new TileData()
            {
                Type = 90,
                Name = "Bathtubs",
            },
            [91] = new TileData()
            {
                Type = 91,
                Name = "Banners",
            },
            [92] = new TileData()
            {
                Type = 92,
                Name = "Lamp_Post",
            },
            [93] = new TileData()
            {
                Type = 93,
                Name = "Lamps",
            },
            [94] = new TileData()
            {
                Type = 94,
                Name = "Keg",
            },
            [95] = new TileData()
            {
                Type = 95,
                Name = "Chinese_Lantern",
            },
            [96] = new TileData()
            {
                Type = 96,
                Name = "Cooking_Pots",
            },
            [97] = new TileData()
            {
                Type = 97,
                Name = "Safe",
            },
            [98] = new TileData()
            {
                Type = 98,
                Name = "Skull_Lantern",
            },
            [99] = new TileData()
            {
                Type = 99,
                Name = "Trash_Can",
            },
            [100] = new TileData()
            {
                Type = 100,
                Name = "Candelabras",
            },
            [101] = new TileData()
            {
                Type = 101,
                Name = "Bookcases",
            },
            [102] = new TileData()
            {
                Type = 102,
                Name = "Throne",
            },
            [103] = new TileData()
            {
                Type = 103,
                Name = "Bowls",
            },
            [104] = new TileData()
            {
                Type = 104,
                Name = "Clocks",
            },
            [105] = new TileData()
            {
                Type = 105,
                Name = "Statue",
            },
            [106] = new TileData()
            {
                Type = 106,
                Name = "Sawmill",
            },
            [107] = new TileData()
            {
                Type = 107,
                Name = "Cobalt_Ore",
                Solid = true,
            },
            [108] = new TileData()
            {
                Type = 108,
                Name = "Mythril_Ore",
                Solid = true,
            },
            [109] = new TileData()
            {
                Type = 109,
                Name = "Hallowed_Grass_Block",
                Solid = true,
            },
            [110] = new TileData()
            {
                Type = 110,
                Name = "Hallow_Short_Plants",
                TextureGrid = new(16, 20),
            },
            [111] = new TileData()
            {
                Type = 111,
                Name = "Adamantite_Ore",
                Solid = true,
            },
            [112] = new TileData()
            {
                Type = 112,
                Name = "Ebonsand_Block",
                Solid = true,
            },
            [113] = new TileData()
            {
                Type = 113,
                Name = "Hallow_Tall_Plants",
                TextureGrid = new(16, 32),
            },
            [114] = new TileData()
            {
                Type = 114,
                Name = "Tinkerer's_Workshop",
            },
            [115] = new TileData()
            {
                Type = 115,
                Name = "Hallowed_Vines",
                Solid = true,
            },
            [116] = new TileData()
            {
                Type = 116,
                Name = "Pearlsand_Block",
                Solid = true,
            },
            [117] = new TileData()
            {
                Type = 117,
                Name = "Pearlstone_Block",
                Solid = true,
            },
            [118] = new TileData()
            {
                Type = 118,
                Name = "Pearlstone_Brick",
                Solid = true,
            },
            [119] = new TileData()
            {
                Type = 119,
                Name = "Iridescent_Brick",
                Solid = true,
            },
            [120] = new TileData()
            {
                Type = 120,
                Name = "Mudstone_Brick",
                Solid = true,
            },
            [121] = new TileData()
            {
                Type = 121,
                Name = "Cobalt_Brick",
                Solid = true,
            },
            [122] = new TileData()
            {
                Type = 122,
                Name = "Mythril_Brick",
                Solid = true,
            },
            [123] = new TileData()
            {
                Type = 123,
                Name = "Silt_Block",
                Solid = true,
            },
            [124] = new TileData()
            {
                Type = 124,
                Name = "Wooden_Beam",
            },
            [125] = new TileData()
            {
                Type = 125,
                Name = "Crystal_Ball",
            },
            [126] = new TileData()
            {
                Type = 126,
                Name = "Disco_Ball",
            },
            [127] = new TileData()
            {
                Type = 127,
                Name = "Ice_(Ice_Rod)",
                Solid = true,
            },
            [128] = new TileData()
            {
                Type = 128,
                Name = "Mannequin_(Legacy)",
            },
            [129] = new TileData()
            {
                Type = 129,
                Name = "Crystal_Shard",
            },
            [130] = new TileData()
            {
                Type = 130,
                Name = "Active_Stone_Block",
                Solid = true,
            },
            [131] = new TileData()
            {
                Type = 131,
                Name = "Inactive_Stone_Block",
                SaveSlope = true,
            },
            [132] = new TileData()
            {
                Type = 132,
                Name = "Lever",
            },
            [133] = new TileData()
            {
                Type = 133,
                Name = "Forges",
            },
            [134] = new TileData()
            {
                Type = 134,
                Name = "Anvils_(Hardmode)",
            },
            [135] = new TileData()
            {
                Type = 135,
                Name = "Pressure_Plates",
            },
            [136] = new TileData()
            {
                Type = 136,
                Name = "Switch",
            },
            [137] = new TileData()
            {
                Type = 137,
                Name = "Traps",
                Solid = true,
            },
            [138] = new TileData()
            {
                Type = 138,
                Name = "Boulder",
                Solid = true,
            },
            [139] = new TileData()
            {
                Type = 139,
                Name = "Music_Boxes",
            },
            [140] = new TileData()
            {
                Type = 140,
                Name = "Demonite_Brick",
                Solid = true,
            },
            [141] = new TileData()
            {
                Type = 141,
                Name = "Explosives",
            },
            [142] = new TileData()
            {
                Type = 142,
                Name = "Inlet_Pump",
            },
            [143] = new TileData()
            {
                Type = 143,
                Name = "Outlet_Pump",
            },
            [144] = new TileData()
            {
                Type = 144,
                Name = "Timers",
            },
            [145] = new TileData()
            {
                Type = 145,
                Name = "Red_Candy_Cane_Block",
                Solid = true,
            },
            [146] = new TileData()
            {
                Type = 146,
                Name = "Green_Candy_Cane_Block",
                Solid = true,
            },
            [147] = new TileData()
            {
                Type = 147,
                Name = "Snow_Block",
                Solid = true,
            },
            [148] = new TileData()
            {
                Type = 148,
                Name = "Snow_Brick",
                Solid = true,
            },
            [149] = new TileData()
            {
                Type = 149,
                Name = "Holiday_Lights",
            },
            [150] = new TileData()
            {
                Type = 150,
                Name = "Adamantite_Beam",
                Solid = true,
            },
            [151] = new TileData()
            {
                Type = 151,
                Name = "Sandstone_Brick",
                Solid = true,
            },
            [152] = new TileData()
            {
                Type = 152,
                Name = "Ebonstone_Brick",
                Solid = true,
            },
            [153] = new TileData()
            {
                Type = 153,
                Name = "Red_Stucco",
                Solid = true,
            },
            [154] = new TileData()
            {
                Type = 154,
                Name = "Yellow_Stucco",
                Solid = true,
            },
            [155] = new TileData()
            {
                Type = 155,
                Name = "Green_Stucco",
                Solid = true,
            },
            [156] = new TileData()
            {
                Type = 156,
                Name = "Gray_Stucco",
                Solid = true,
            },
            [157] = new TileData()
            {
                Type = 157,
                Name = "Ebonwood",
                Solid = true,
            },
            [158] = new TileData()
            {
                Type = 158,
                Name = "Rich_Mahogany",
                Solid = true,
            },
            [159] = new TileData()
            {
                Type = 159,
                Name = "Pearlwood",
                Solid = true,
            },
            [160] = new TileData()
            {
                Type = 160,
                Name = "Rainbow_Brick",
                Solid = true,
            },
            [161] = new TileData()
            {
                Type = 161,
                Name = "Ice_Block",
                Solid = true,
            },
            [162] = new TileData()
            {
                Type = 162,
                Name = "Thin_Ice_Block",
                Solid = true,
            },
            [163] = new TileData()
            {
                Type = 163,
                Name = "Purple_Ice_Block",
                Solid = true,
            },
            [164] = new TileData()
            {
                Type = 164,
                Name = "Pink_Ice_Block",
                Solid = true,
            },
            [165] = new TileData()
            {
                Type = 165,
                Name = "Cave_Decos",
            },
            [166] = new TileData()
            {
                Type = 166,
                Name = "Tin_Ore",
                Solid = true,
            },
            [167] = new TileData()
            {
                Type = 167,
                Name = "Lead_Ore",
                Solid = true,
            },
            [168] = new TileData()
            {
                Type = 168,
                Name = "Tungsten_Ore",
                Solid = true,
            },
            [169] = new TileData()
            {
                Type = 169,
                Name = "Platinum_Ore",
                Solid = true,
            },
            [170] = new TileData()
            {
                Type = 170,
                Name = "Pine_Tree_Block",
                Solid = true,
            },
            [171] = new TileData()
            {
                Type = 171,
                Name = "Christmas_Tree",
            },
            [172] = new TileData()
            {
                Type = 172,
                Name = "Sinks",
            },
            [173] = new TileData()
            {
                Type = 173,
                Name = "Platinum_Candelabra",
            },
            [174] = new TileData()
            {
                Type = 174,
                Name = "Platinum_Candle",
            },
            [175] = new TileData()
            {
                Type = 175,
                Name = "Tin_Brick",
                Solid = true,
            },
            [176] = new TileData()
            {
                Type = 176,
                Name = "Tungsten_Brick",
                Solid = true,
            },
            [177] = new TileData()
            {
                Type = 177,
                Name = "Platinum_Brick",
                Solid = true,
            },
            [178] = new TileData()
            {
                Type = 178,
                Name = "Gems",
            },
            [179] = new TileData()
            {
                Type = 179,
                Name = "Green_Moss_Block",
                Solid = true,
            },
            [180] = new TileData()
            {
                Type = 180,
                Name = "Yellow_Moss_Block",
                Solid = true,
            },
            [181] = new TileData()
            {
                Type = 181,
                Name = "Red_Moss_Block",
                Solid = true,
            },
            [182] = new TileData()
            {
                Type = 182,
                Name = "Blue_Moss_Block",
                Solid = true,
            },
            [183] = new TileData()
            {
                Type = 183,
                Name = "Purple_Moss_Block",
                Solid = true,
            },
            [184] = new TileData()
            {
                Type = 184,
                Name = "Moss_Plants",
                TextureGrid = new(20, 16),
            },
            [185] = new TileData()
            {
                Type = 185,
                Name = "Small_Decos",
            },
            [186] = new TileData()
            {
                Type = 186,
                Name = "Decos_(3x2)",
            },
            [187] = new TileData()
            {
                Type = 187,
                Name = "Decos_(3x2)_(Group_2)",
            },
            [188] = new TileData()
            {
                Type = 188,
                Name = "Cactus_Block",
                Solid = true,
            },
            [189] = new TileData()
            {
                Type = 189,
                Name = "Cloud_Block",
                Solid = true,
            },
            [190] = new TileData()
            {
                Type = 190,
                Name = "Glowing_Mushroom_Block",
                Solid = true,
            },
            [191] = new TileData()
            {
                Type = 191,
                Name = "Living_Wood_Block",
                Solid = true,
            },
            [192] = new TileData()
            {
                Type = 192,
                Name = "Living_Leaf_Block",
                Solid = true,
            },
            [193] = new TileData()
            {
                Type = 193,
                Name = "Slime_Block",
                Solid = true,
            },
            [194] = new TileData()
            {
                Type = 194,
                Name = "Bone_Block",
                Solid = true,
            },
            [195] = new TileData()
            {
                Type = 195,
                Name = "Flesh_Block",
                Solid = true,
            },
            [196] = new TileData()
            {
                Type = 196,
                Name = "Rain_Cloud_Block",
                Solid = true,
            },
            [197] = new TileData()
            {
                Type = 197,
                Name = "Frozen_Slime_Block",
                Solid = true,
            },
            [198] = new TileData()
            {
                Type = 198,
                Name = "Asphalt_Block",
                Solid = true,
            },
            [199] = new TileData()
            {
                Type = 199,
                Name = "Crimson_Grass_Block",
                Solid = true,
            },
            [200] = new TileData()
            {
                Type = 200,
                Name = "Red_Ice_Block",
                Solid = true,
            },
            [201] = new TileData()
            {
                Type = 201,
                Name = "Crimson_Short_Plants",
            },
            [202] = new TileData()
            {
                Type = 202,
                Name = "Sunplate_Block",
                Solid = true,
            },
            [203] = new TileData()
            {
                Type = 203,
                Name = "Crimstone_Block",
                Solid = true,
            },
            [204] = new TileData()
            {
                Type = 204,
                Name = "Crimtane_Ore",
                Solid = true,
            },
            [205] = new TileData()
            {
                Type = 205,
                Name = "Crimson_Vines",
                Solid = true,
            },
            [206] = new TileData()
            {
                Type = 206,
                Name = "Ice_Brick",
                Solid = true,
            },
            [207] = new TileData()
            {
                Type = 207,
                Name = "Water_Fountains",
            },
            [208] = new TileData()
            {
                Type = 208,
                Name = "Shadewood",
                Solid = true,
            },
            [209] = new TileData()
            {
                Type = 209,
                Name = "Cannons",
            },
            [210] = new TileData()
            {
                Type = 210,
                Name = "Land_Mine",
            },
            [211] = new TileData()
            {
                Type = 211,
                Name = "Chlorophyte_Ore",
                Solid = true,
            },
            [212] = new TileData()
            {
                Type = 212,
                Name = "Snowball_Launcher",
            },
            [213] = new TileData()
            {
                Type = 213,
                Name = "Rope",
            },
            [214] = new TileData()
            {
                Type = 214,
                Name = "Chain",
            },
            [215] = new TileData()
            {
                Type = 215,
                Name = "Campfires",
            },
            [216] = new TileData()
            {
                Type = 216,
                Name = "Rockets",
            },
            [217] = new TileData()
            {
                Type = 217,
                Name = "Blend-O-Matic",
            },
            [218] = new TileData()
            {
                Type = 218,
                Name = "Meat_Grinder",
            },
            [219] = new TileData()
            {
                Type = 219,
                Name = "Extractinator",
            },
            [220] = new TileData()
            {
                Type = 220,
                Name = "Solidifier",
            },
            [221] = new TileData()
            {
                Type = 221,
                Name = "Palladium_Ore",
                Solid = true,
            },
            [222] = new TileData()
            {
                Type = 222,
                Name = "Orichalcum_Ore",
                Solid = true,
            },
            [223] = new TileData()
            {
                Type = 223,
                Name = "Titanium_Ore",
                Solid = true,
            },
            [224] = new TileData()
            {
                Type = 224,
                Name = "Slush_Block",
                Solid = true,
            },
            [225] = new TileData()
            {
                Type = 225,
                Name = "Hive_Block",
                Solid = true,
            },
            [226] = new TileData()
            {
                Type = 226,
                Name = "Lihzahrd_Brick",
                Solid = true,
            },
            [227] = new TileData()
            {
                Type = 227,
                Name = "Dye_Plants",
                TextureGrid = new(32, 38),
            },
            [228] = new TileData()
            {
                Type = 228,
                Name = "Dye_Vat",
            },
            [229] = new TileData()
            {
                Type = 229,
                Name = "Honey_Block",
                Solid = true,
            },
            [230] = new TileData()
            {
                Type = 230,
                Name = "Crispy_Honey_Block",
                Solid = true,
            },
            [231] = new TileData()
            {
                Type = 231,
                Name = "Larva",
            },
            [232] = new TileData()
            {
                Type = 232,
                Name = "Wooden_Spike",
                Solid = true,
            },
            [233] = new TileData()
            {
                Type = 233,
                Name = "Jungle_Large_Plants",
            },
            [234] = new TileData()
            {
                Type = 234,
                Name = "Crimsand_Block",
                Solid = true,
            },
            [235] = new TileData()
            {
                Type = 235,
                Name = "Teleporter",
                Solid = true,
            },
            [236] = new TileData()
            {
                Type = 236,
                Name = "Life_Fruit_Plant",
            },
            [237] = new TileData()
            {
                Type = 237,
                Name = "Lihzahrd_Altar",
            },
            [238] = new TileData()
            {
                Type = 238,
                Name = "Plantera's_Bulb",
            },
            [239] = new TileData()
            {
                Type = 239,
                Name = "Ore_Bars",
                Solid = true,
            },
            [240] = new TileData()
            {
                Type = 240,
                Name = "Wall_Hangings_3x3",
            },
            [241] = new TileData()
            {
                Type = 241,
                Name = "Catacombs",
            },
            [242] = new TileData()
            {
                Type = 242,
                Name = "Paintings_6x4",
            },
            [243] = new TileData()
            {
                Type = 243,
                Name = "Imbuing_Station",
            },
            [244] = new TileData()
            {
                Type = 244,
                Name = "Bubble_Machine",
            },
            [245] = new TileData()
            {
                Type = 245,
                Name = "Paintings_2x3",
            },
            [246] = new TileData()
            {
                Type = 246,
                Name = "Paintings_3x2",
            },
            [247] = new TileData()
            {
                Type = 247,
                Name = "Autohammer",
            },
            [248] = new TileData()
            {
                Type = 248,
                Name = "Palladium_Column",
                Solid = true,
            },
            [249] = new TileData()
            {
                Type = 249,
                Name = "Bubblegum_Block",
                Solid = true,
            },
            [250] = new TileData()
            {
                Type = 250,
                Name = "Titanstone_Block",
                Solid = true,
            },
            [251] = new TileData()
            {
                Type = 251,
                Name = "Pumpkin_Block",
                Solid = true,
            },
            [252] = new TileData()
            {
                Type = 252,
                Name = "Hay_Block",
                Solid = true,
            },
            [253] = new TileData()
            {
                Type = 253,
                Name = "Spooky_Wood",
                Solid = true,
            },
            [254] = new TileData()
            {
                Type = 254,
                Name = "Pumpkin_Plant",
            },
            [255] = new TileData()
            {
                Type = 255,
                Name = "Amethyst_Gemspark_Block_(Offline)",
                Solid = true,
            },
            [256] = new TileData()
            {
                Type = 256,
                Name = "Topaz_Gemspark_Block_(Offline)",
                Solid = true,
            },
            [257] = new TileData()
            {
                Type = 257,
                Name = "Sapphire_Gemspark_Block_(Offline)",
                Solid = true,
            },
            [258] = new TileData()
            {
                Type = 258,
                Name = "Emerald_Gemspark_Block_(Offline)",
                Solid = true,
            },
            [259] = new TileData()
            {
                Type = 259,
                Name = "Ruby_Gemspark_Block_(Offline)",
                Solid = true,
            },
            [260] = new TileData()
            {
                Type = 260,
                Name = "Diamond_Gemspark_Block_(Offline)",
                Solid = true,
            },
            [261] = new TileData()
            {
                Type = 261,
                Name = "Amber_Gemspark_Block_(Offline)",
                Solid = true,
            },
            [262] = new TileData()
            {
                Type = 262,
                Name = "Amethyst_Gemspark_Block",
                Solid = true,
            },
            [263] = new TileData()
            {
                Type = 263,
                Name = "Topaz_Gemspark_Block",
                Solid = true,
            },
            [264] = new TileData()
            {
                Type = 264,
                Name = "Sapphire_Gemspark_Block",
                Solid = true,
            },
            [265] = new TileData()
            {
                Type = 265,
                Name = "Emerald_Gemspark_Block",
                Solid = true,
            },
            [266] = new TileData()
            {
                Type = 266,
                Name = "Ruby_Gemspark_Block",
                Solid = true,
            },
            [267] = new TileData()
            {
                Type = 267,
                Name = "Diamond_Gemspark_Block",
                Solid = true,
            },
            [268] = new TileData()
            {
                Type = 268,
                Name = "Amber_Gemspark_Block",
                Solid = true,
            },
            [269] = new TileData()
            {
                Type = 269,
                Name = "Womannequin_(Legacy)",
            },
            [270] = new TileData()
            {
                Type = 270,
                Name = "Firefly_in_a_Bottle",
            },
            [271] = new TileData()
            {
                Type = 271,
                Name = "Lightning_Bug_in_a_Bottle",
            },
            [272] = new TileData()
            {
                Type = 272,
                Name = "Cog_Block",
                Solid = true,
            },
            [273] = new TileData()
            {
                Type = 273,
                Name = "Stone_Slab",
                Solid = true,
            },
            [274] = new TileData()
            {
                Type = 274,
                Name = "Sandstone_Slab",
                Solid = true,
            },
            [275] = new TileData()
            {
                Type = 275,
                Name = "Bunny_Cage",
            },
            [276] = new TileData()
            {
                Type = 276,
                Name = "Squirrel_Cage",
            },
            [277] = new TileData()
            {
                Type = 277,
                Name = "Mallard_Duck_Cage",
            },
            [278] = new TileData()
            {
                Type = 278,
                Name = "Duck_Cage",
            },
            [279] = new TileData()
            {
                Type = 279,
                Name = "Bird_Cage",
            },
            [280] = new TileData()
            {
                Type = 280,
                Name = "Blue_Jay_Cage",
            },
            [281] = new TileData()
            {
                Type = 281,
                Name = "Cardinal_Cage",
            },
            [282] = new TileData()
            {
                Type = 282,
                Name = "Fish_Bowl",
            },
            [283] = new TileData()
            {
                Type = 283,
                Name = "Heavy_Work_Bench",
            },
            [284] = new TileData()
            {
                Type = 284,
                Name = "Copper_Plating",
                Solid = true,
            },
            [285] = new TileData()
            {
                Type = 285,
                Name = "Snail_Cage",
            },
            [286] = new TileData()
            {
                Type = 286,
                Name = "Glowing_Snail_Cage",
            },
            [287] = new TileData()
            {
                Type = 287,
                Name = "Ammo_Box",
            },
            [288] = new TileData()
            {
                Type = 288,
                Name = "Monarch_Butterfly_Jar",
            },
            [289] = new TileData()
            {
                Type = 289,
                Name = "Purple_Emperor_Butterfly_Jar",
            },
            [290] = new TileData()
            {
                Type = 290,
                Name = "Red_Admiral_Butterfly_Jar",
            },
            [291] = new TileData()
            {
                Type = 291,
                Name = "Ulysses_Butterfly_Jar",
            },
            [292] = new TileData()
            {
                Type = 292,
                Name = "Sulphur_Butterfly_Jar",
            },
            [293] = new TileData()
            {
                Type = 293,
                Name = "Tree_Nymph_Butterfly_Jar",
            },
            [294] = new TileData()
            {
                Type = 294,
                Name = "Zebra_Swallowtail_Butterfly_Jar",
            },
            [295] = new TileData()
            {
                Type = 295,
                Name = "Julia_Butterfly_Jar",
            },
            [296] = new TileData()
            {
                Type = 296,
                Name = "Scorpion_Cage",
            },
            [297] = new TileData()
            {
                Type = 297,
                Name = "Black_Scorpion_Cage",
            },
            [298] = new TileData()
            {
                Type = 298,
                Name = "Frog_Cage",
            },
            [299] = new TileData()
            {
                Type = 299,
                Name = "Mouse_Cage",
            },
            [300] = new TileData()
            {
                Type = 300,
                Name = "Bone_Welder",
            },
            [301] = new TileData()
            {
                Type = 301,
                Name = "Flesh_Cloning_Vat",
            },
            [302] = new TileData()
            {
                Type = 302,
                Name = "Glass_Kiln",
            },
            [303] = new TileData()
            {
                Type = 303,
                Name = "Lihzahrd_Furnace",
            },
            [304] = new TileData()
            {
                Type = 304,
                Name = "Living_Loom",
            },
            [305] = new TileData()
            {
                Type = 305,
                Name = "Sky_Mill",
            },
            [306] = new TileData()
            {
                Type = 306,
                Name = "Ice_Machine",
            },
            [307] = new TileData()
            {
                Type = 307,
                Name = "Steampunk_Boiler",
            },
            [308] = new TileData()
            {
                Type = 308,
                Name = "Honey_Dispenser",
            },
            [309] = new TileData()
            {
                Type = 309,
                Name = "Penguin_Cage",
            },
            [310] = new TileData()
            {
                Type = 310,
                Name = "Worm_Cage",
            },
            [311] = new TileData()
            {
                Type = 311,
                Name = "Dynasty_Wood",
                Solid = true,
            },
            [312] = new TileData()
            {
                Type = 312,
                Name = "Red_Dynasty_Shingles",
                Solid = true,
            },
            [313] = new TileData()
            {
                Type = 313,
                Name = "Blue_Dynasty_Shingles",
                Solid = true,
            },
            [314] = new TileData()
            {
                Type = 314,
                Name = "Minecart_Track",
            },
            [315] = new TileData()
            {
                Type = 315,
                Name = "Coralstone_Block",
                Solid = true,
            },
            [316] = new TileData()
            {
                Type = 316,
                Name = "Blue_Jellyfish_Jar",
            },
            [317] = new TileData()
            {
                Type = 317,
                Name = "Green_Jellyfish_Jar",
            },
            [318] = new TileData()
            {
                Type = 318,
                Name = "Pink_Jellyfish_Jar",
            },
            [319] = new TileData()
            {
                Type = 319,
                Name = "Ship_in_a_Bottle",
            },
            [320] = new TileData()
            {
                Type = 320,
                Name = "Seaweed_Planter",
            },
            [321] = new TileData()
            {
                Type = 321,
                Name = "Boreal_Wood",
                Solid = true,
            },
            [322] = new TileData()
            {
                Type = 322,
                Name = "Palm_Wood",
                Solid = true,
            },
            [323] = new TileData()
            {
                Type = 323,
                Name = "Palm_Trees",
                TextureGrid = new(20, 20),
            },
            [324] = new TileData()
            {
                Type = 324,
                Name = "Seashells",
                TextureGrid = new(20, 20),
            },
            [325] = new TileData()
            {
                Type = 325,
                Name = "Tin_Plating",
                Solid = true,
            },
            [326] = new TileData()
            {
                Type = 326,
                Name = "Waterfall_Block",
                Solid = true,
            },
            [327] = new TileData()
            {
                Type = 327,
                Name = "Lavafall_Block",
                Solid = true,
            },
            [328] = new TileData()
            {
                Type = 328,
                Name = "Confetti_Block",
                Solid = true,
            },
            [329] = new TileData()
            {
                Type = 329,
                Name = "Midnight_Confetti_Block",
                Solid = true,
            },
            [330] = new TileData()
            {
                Type = 330,
                Name = "Copper_Coin_Pile",
            },
            [331] = new TileData()
            {
                Type = 331,
                Name = "Silver_Coin_Pile",
            },
            [332] = new TileData()
            {
                Type = 332,
                Name = "Gold_Coin_Pile",
            },
            [333] = new TileData()
            {
                Type = 333,
                Name = "Platinum_Coin_Pile",
            },
            [334] = new TileData()
            {
                Type = 334,
                Name = "Weapon_Rack_(Legacy_-_DO_NOT_USE)",
            },
            [335] = new TileData()
            {
                Type = 335,
                Name = "Fireworks_Box",
            },
            [336] = new TileData()
            {
                Type = 336,
                Name = "Living_Fire_Block",
                Solid = true,
            },
            [337] = new TileData()
            {
                Type = 337,
                Name = "Text_Statue",
            },
            [338] = new TileData()
            {
                Type = 338,
                Name = "Firework_Fountain",
            },
            [339] = new TileData()
            {
                Type = 339,
                Name = "Grasshopper_Cage",
            },
            [340] = new TileData()
            {
                Type = 340,
                Name = "Living_Cursed_Fire_Block",
                Solid = true,
            },
            [341] = new TileData()
            {
                Type = 341,
                Name = "Living_Demon_Fire_Block",
                Solid = true,
            },
            [342] = new TileData()
            {
                Type = 342,
                Name = "Living_Frost_Fire_Block",
                Solid = true,
            },
            [343] = new TileData()
            {
                Type = 343,
                Name = "Living_Ichor_Fire_Block",
                Solid = true,
            },
            [344] = new TileData()
            {
                Type = 344,
                Name = "Living_Ultrabright_Fire_Block",
                Solid = true,
            },
            [345] = new TileData()
            {
                Type = 345,
                Name = "Honeyfall_Block",
                Solid = true,
            },
            [346] = new TileData()
            {
                Type = 346,
                Name = "Chlorophyte_Brick",
                Solid = true,
            },
            [347] = new TileData()
            {
                Type = 347,
                Name = "Crimtane_Brick",
                Solid = true,
            },
            [348] = new TileData()
            {
                Type = 348,
                Name = "Shroomite_Plating",
                Solid = true,
            },
            [349] = new TileData()
            {
                Type = 349,
                Name = "Mushroom_Statue",
            },
            [350] = new TileData()
            {
                Type = 350,
                Name = "Martian_Conduit_Plating",
                Solid = true,
            },
            [351] = new TileData()
            {
                Type = 351,
                Name = "Smoke_Block",
                Solid = true,
            },
            [352] = new TileData()
            {
                Type = 352,
                Name = "Crimtane_Thorns",
                Solid = true,
            },
            [353] = new TileData()
            {
                Type = 353,
                Name = "Vine_Rope",
                Solid = true,
            },
            [354] = new TileData()
            {
                Type = 354,
                Name = "Bewitching_Table",
            },
            [355] = new TileData()
            {
                Type = 355,
                Name = "Alchemy_Table",
            },
            [356] = new TileData()
            {
                Type = 356,
                Name = "Enchanted_Sundial",
            },
            [357] = new TileData()
            {
                Type = 357,
                Name = "Smooth_Marble_Block",
                Solid = true,
            },
            [358] = new TileData()
            {
                Type = 358,
                Name = "Gold_Bird_Cage",
            },
            [359] = new TileData()
            {
                Type = 359,
                Name = "Gold_Bunny_Cage",
            },
            [360] = new TileData()
            {
                Type = 360,
                Name = "Gold_Butterfly_Cage",
            },
            [361] = new TileData()
            {
                Type = 361,
                Name = "Gold_Frog_Cage",
            },
            [362] = new TileData()
            {
                Type = 362,
                Name = "Gold_Grasshopper_Cage",
            },
            [363] = new TileData()
            {
                Type = 363,
                Name = "Gold_Mouse_Cage",
            },
            [364] = new TileData()
            {
                Type = 364,
                Name = "Gold_Worm_Cage",
            },
            [365] = new TileData()
            {
                Type = 365,
                Name = "Silk_Rope",
                Solid = true,
            },
            [366] = new TileData()
            {
                Type = 366,
                Name = "Web_Rope",
                Solid = true,
            },
            [367] = new TileData()
            {
                Type = 367,
                Name = "Marble_Block",
                Solid = true,
            },
            [368] = new TileData()
            {
                Type = 368,
                Name = "Granite_Block",
                Solid = true,
            },
            [369] = new TileData()
            {
                Type = 369,
                Name = "Smooth_Granite_Block",
                Solid = true,
            },
            [370] = new TileData()
            {
                Type = 370,
                Name = "Meteorite_Brick",
                Solid = true,
            },
            [371] = new TileData()
            {
                Type = 371,
                Name = "Pink_Slime_Block",
                Solid = true,
            },
            [372] = new TileData()
            {
                Type = 372,
                Name = "Peace_Candle",
            },
            [373] = new TileData()
            {
                Type = 373,
                Name = "Water_Drip",
            },
            [374] = new TileData()
            {
                Type = 374,
                Name = "Lava_Drip",
            },
            [375] = new TileData()
            {
                Type = 375,
                Name = "Honey_Drip",
            },
            [376] = new TileData()
            {
                Type = 376,
                Name = "Fishing_Crates",
            },
            [377] = new TileData()
            {
                Type = 377,
                Name = "Sharpening_Station",
            },
            [378] = new TileData()
            {
                Type = 378,
                Name = "Target_Dummy",
            },
            [379] = new TileData()
            {
                Type = 379,
                Name = "Bubble",
                Solid = true,
            },
            [380] = new TileData()
            {
                Type = 380,
                Name = "Planter_Boxes",
                Solid = true,
            },
            [381] = new TileData()
            {
                Type = 381,
                Name = "Lava_Moss",
                Solid = true,
            },
            [382] = new TileData()
            {
                Type = 382,
                Name = "Flower_Vines",
            },
            [383] = new TileData()
            {
                Type = 383,
                Name = "Living_Mahogany_Block",
                Solid = true,
            },
            [384] = new TileData()
            {
                Type = 384,
                Name = "Living_Mahogany_Leaves_Block",
                Solid = true,
            },
            [385] = new TileData()
            {
                Type = 385,
                Name = "Crystal_Block",
                Solid = true,
            },
            [386] = new TileData()
            {
                Type = 386,
                Name = "Trap_Door_(Open)",
            },
            [387] = new TileData()
            {
                Type = 387,
                Name = "Trap_Door_(Closed)",
                Solid = true,
            },
            [388] = new TileData()
            {
                Type = 388,
                Name = "Tall_Gate_(Closed)",
                TextureGrid = new(16, 18),
                Solid = true,
            },
            [389] = new TileData()
            {
                Type = 389,
                Name = "Tall_Gate_(Open)",
                TextureGrid = new(16, 18),
            },
            [390] = new TileData()
            {
                Type = 390,
                Name = "Lava_Lamp",
            },
            [391] = new TileData()
            {
                Type = 391,
                Name = "Cage_Enchanted_Nightcrawler",
            },
            [392] = new TileData()
            {
                Type = 392,
                Name = "Buggy_Cage",
            },
            [393] = new TileData()
            {
                Type = 393,
                Name = "Grubby_Cage",
            },
            [394] = new TileData()
            {
                Type = 394,
                Name = "Sluggy_Cage",
            },
            [395] = new TileData()
            {
                Type = 395,
                Name = "Item_Frame",
            },
            [396] = new TileData()
            {
                Type = 396,
                Name = "Sandstone_Block",
                Solid = true,
            },
            [397] = new TileData()
            {
                Type = 397,
                Name = "Hardened_Sand_Block",
                Solid = true,
            },
            [398] = new TileData()
            {
                Type = 398,
                Name = "Corrupt_Hardened_Sand_Block",
                Solid = true,
            },
            [399] = new TileData()
            {
                Type = 399,
                Name = "Crimson_Hardened_Sand_Block",
                Solid = true,
            },
            [400] = new TileData()
            {
                Type = 400,
                Name = "Corrupt_Sandstone_Block",
                Solid = true,
            },
            [401] = new TileData()
            {
                Type = 401,
                Name = "Crimson_Sandstone_Block",
                Solid = true,
            },
            [402] = new TileData()
            {
                Type = 402,
                Name = "Hallow_Hardened_Sand_Block",
                Solid = true,
            },
            [403] = new TileData()
            {
                Type = 403,
                Name = "Hallow_Sandstone_Block",
                Solid = true,
            },
            [404] = new TileData()
            {
                Type = 404,
                Name = "Desert_Fossil_Block",
                Solid = true,
            },
            [405] = new TileData()
            {
                Type = 405,
                Name = "Fireplace",
            },
            [406] = new TileData()
            {
                Type = 406,
                Name = "Chimney",
            },
            [407] = new TileData()
            {
                Type = 407,
                Name = "Sturdy_Fossil_Block",
                Solid = true,
            },
            [408] = new TileData()
            {
                Type = 408,
                Name = "Luminite_Ore",
                Solid = true,
            },
            [409] = new TileData()
            {
                Type = 409,
                Name = "Luminite_Brick",
                Solid = true,
            },
            [410] = new TileData()
            {
                Type = 410,
                Name = "Lunar_Monoliths",
            },
            [411] = new TileData()
            {
                Type = 411,
                Name = "Detonator",
            },
            [412] = new TileData()
            {
                Type = 412,
                Name = "Lunar_Crafting_Station",
            },
            [413] = new TileData()
            {
                Type = 413,
                Name = "Red_Squirrel_Cage",
            },
            [414] = new TileData()
            {
                Type = 414,
                Name = "Gold_Squirrel_Cage",
            },
            [415] = new TileData()
            {
                Type = 415,
                Name = "Solar_Fragment_Block",
                Solid = true,
            },
            [416] = new TileData()
            {
                Type = 416,
                Name = "Vortex_Fragment_Block",
                Solid = true,
            },
            [417] = new TileData()
            {
                Type = 417,
                Name = "Nebula_Fragment_Block",
                Solid = true,
            },
            [418] = new TileData()
            {
                Type = 418,
                Name = "Stardust_Fragment_Block",
                Solid = true,
            },
            [419] = new TileData()
            {
                Type = 419,
                Name = "Logic_Lamp",
            },
            [420] = new TileData()
            {
                Type = 420,
                Name = "Logic_Gates",
            },
            [421] = new TileData()
            {
                Type = 421,
                Name = "Conveyor_Belt_(CW)",
                Solid = true,
            },
            [422] = new TileData()
            {
                Type = 422,
                Name = "Conveyor_Belt_(CCW)",
                Solid = true,
            },
            [423] = new TileData()
            {
                Type = 423,
                Name = "Logic_Sensor",
            },
            [424] = new TileData()
            {
                Type = 424,
                Name = "Junction_Box",
            },
            [425] = new TileData()
            {
                Type = 425,
                Name = "Announcement_Box",
            },
            [426] = new TileData()
            {
                Type = 426,
                Name = "Red_Team_Block",
                Solid = true,
            },
            [427] = new TileData()
            {
                Type = 427,
                Name = "Red_Team_Platform",
                Solid = true,
            },
            [428] = new TileData()
            {
                Type = 428,
                Name = "Weighted_Pressure_Plates",
            },
            [429] = new TileData()
            {
                Type = 429,
                Name = "Wire_Bulb",
            },
            [430] = new TileData()
            {
                Type = 430,
                Name = "Green_Team_Block",
                Solid = true,
            },
            [431] = new TileData()
            {
                Type = 431,
                Name = "Blue_Team_Block",
                Solid = true,
            },
            [432] = new TileData()
            {
                Type = 432,
                Name = "Yellow_Team_Block",
                Solid = true,
            },
            [433] = new TileData()
            {
                Type = 433,
                Name = "Pink_Team_Block",
                Solid = true,
            },
            [434] = new TileData()
            {
                Type = 434,
                Name = "White_Team_Block",
                Solid = true,
            },
            [435] = new TileData()
            {
                Type = 435,
                Name = "Green_Team_Platform",
                Solid = true,
            },
            [436] = new TileData()
            {
                Type = 436,
                Name = "Blue_Team_Platform",
                Solid = true,
            },
            [437] = new TileData()
            {
                Type = 437,
                Name = "Yellow_Team_Platform",
                Solid = true,
            },
            [438] = new TileData()
            {
                Type = 438,
                Name = "Pink_Team_Platform",
                Solid = true,
            },
            [439] = new TileData()
            {
                Type = 439,
                Name = "White_Team_Platform",
                Solid = true,
            },
            [440] = new TileData()
            {
                Type = 440,
                Name = "Gem_Locks",
            },
            [441] = new TileData()
            {
                Type = 441,
                Name = "Trapped_Chests",
            },
            [442] = new TileData()
            {
                Type = 442,
                Name = "Teal_Pressure_Pad",
                TextureGrid = new(20, 20),
            },
            [443] = new TileData()
            {
                Type = 443,
                Name = "Geyser",
            },
            [444] = new TileData()
            {
                Type = 444,
                Name = "Bee_Hive",
            },
            [445] = new TileData()
            {
                Type = 445,
                Name = "Pixel_Box",
            },
            [446] = new TileData()
            {
                Type = 446,
                Name = "Silly_Pink_Balloon_Block",
                Solid = true,
            },
            [447] = new TileData()
            {
                Type = 447,
                Name = "Silly_Purple_Balloon_Block",
                Solid = true,
            },
            [448] = new TileData()
            {
                Type = 448,
                Name = "Silly_Green_Balloon_Block",
                Solid = true,
            },
            [449] = new TileData()
            {
                Type = 449,
                Name = "Blue_Streamer",
                Solid = true,
            },
            [450] = new TileData()
            {
                Type = 450,
                Name = "Green_Streamer",
                Solid = true,
            },
            [451] = new TileData()
            {
                Type = 451,
                Name = "Pink_Streamer",
                Solid = true,
            },
            [452] = new TileData()
            {
                Type = 452,
                Name = "Silly_Balloon_Machine",
            },
            [453] = new TileData()
            {
                Type = 453,
                Name = "Silly_Tied_Balloon",
            },
            [454] = new TileData()
            {
                Type = 454,
                Name = "Pigronata",
            },
            [455] = new TileData()
            {
                Type = 455,
                Name = "Party_Center",
            },
            [456] = new TileData()
            {
                Type = 456,
                Name = "Silly_Tied_Bundle_of_Balloons",
            },
            [457] = new TileData()
            {
                Type = 457,
                Name = "Party_Presents",
            },
            [458] = new TileData()
            {
                Type = 458,
                Name = "Sandfall_Block",
                Solid = true,
            },
            [459] = new TileData()
            {
                Type = 459,
                Name = "Snowfall_Block",
                Solid = true,
            },
            [460] = new TileData()
            {
                Type = 460,
                Name = "Snow_Cloud_Block",
                Solid = true,
            },
            [461] = new TileData()
            {
                Type = 461,
                Name = "Sand_Drip",
            },
            [462] = new TileData()
            {
                Type = 462,
                Name = "Desert_Spirit_Lamp",
            },
            [463] = new TileData()
            {
                Type = 463,
                Name = "Defender's_Forge",
            },
            [464] = new TileData()
            {
                Type = 464,
                Name = "War_Table",
            },
            [465] = new TileData()
            {
                Type = 465,
                Name = "War_Table_Banner",
            },
            [466] = new TileData()
            {
                Type = 466,
                Name = "Elder_Cystal_Stand",
            },
            [467] = new TileData()
            {
                Type = 467,
                Name = "Chests_(Group_2)",
            },
            [468] = new TileData()
            {
                Type = 468,
                Name = "Trapped_Chests_(Group_2)",
            },
            [469] = new TileData()
            {
                Type = 469,
                Name = "Tables_(Group_2)",
            },
            [470] = new TileData()
            {
                Type = 470,
                Name = "Mannequins",
            },
            [471] = new TileData()
            {
                Type = 471,
                Name = "Weapon_Rack",
            },
            [472] = new TileData()
            {
                Type = 472,
                Name = "Iron_Brick",
                Solid = true,
            },
            [473] = new TileData()
            {
                Type = 473,
                Name = "Lead_Brick",
                Solid = true,
            },
            [474] = new TileData()
            {
                Type = 474,
                Name = "Lesion_Block",
                Solid = true,
            },
            [475] = new TileData()
            {
                Type = 475,
                Name = "Hat_Rack",
            },
            [476] = new TileData()
            {
                Type = 476,
                Name = "Golf_Cup",
                TextureGrid = new(20, 18),
                Solid = true,
            },
            [477] = new TileData()
            {
                Type = 477,
                Name = "Mowed_Grass_Block",
                Solid = true,
            },
            [478] = new TileData()
            {
                Type = 478,
                Name = "Crimstone_Brick",
                Solid = true,
            },
            [479] = new TileData()
            {
                Type = 479,
                Name = "Smooth_Sandstone_Block",
                Solid = true,
            },
            [480] = new TileData()
            {
                Type = 480,
                Name = "Blood_Moon_Monolith",
            },
            [481] = new TileData()
            {
                Type = 481,
                Name = "Cracked_Blue_Brick",
                Solid = true,
            },
            [482] = new TileData()
            {
                Type = 482,
                Name = "Cracked_Green_Brick",
                Solid = true,
            },
            [483] = new TileData()
            {
                Type = 483,
                Name = "Cracked_Pink_Brick",
                Solid = true,
            },
            [484] = new TileData()
            {
                Type = 484,
                Name = "Rolling_Cactus",
                Solid = true,
            },
            [485] = new TileData()
            {
                Type = 485,
                Name = "Antlion_Larva",
            },
            [486] = new TileData()
            {
                Type = 486,
                Name = "Drum_Set",
            },
            [487] = new TileData()
            {
                Type = 487,
                Name = "Picnic_Tables",
            },
            [488] = new TileData()
            {
                Type = 488,
                Name = "Fallen_Log",
            },
            [489] = new TileData()
            {
                Type = 489,
                Name = "Pin_Wheel",
            },
            [490] = new TileData()
            {
                Type = 490,
                Name = "Weather_Vane",
            },
            [491] = new TileData()
            {
                Type = 491,
                Name = "Void_Vault",
            },
            [492] = new TileData()
            {
                Type = 492,
                Name = "Hallowed_Mowed_Grass_Block",
                Solid = true,
            },
            [493] = new TileData()
            {
                Type = 493,
                Name = "Pin_Flags",
            },
            [494] = new TileData()
            {
                Type = 494,
                Name = "Golf_Tee",
            },
            [495] = new TileData()
            {
                Type = 495,
                Name = "Shell_Pile_Block",
                Solid = true,
            },
            [496] = new TileData()
            {
                Type = 496,
                Name = "Anti-Portal_Block",
                Solid = true,
            },
            [497] = new TileData()
            {
                Type = 497,
                Name = "Toilets",
            },
            [498] = new TileData()
            {
                Type = 498,
                Name = "Spider_Nest_Block",
                Solid = true,
            },
            [499] = new TileData()
            {
                Type = 499,
                Name = "Decay_Chamber",
            },
            [500] = new TileData()
            {
                Type = 500,
                Name = "Solar_Brick",
                Solid = true,
            },
            [501] = new TileData()
            {
                Type = 501,
                Name = "Vortex_Brick",
                Solid = true,
            },
            [502] = new TileData()
            {
                Type = 502,
                Name = "Nebula_Brick",
                Solid = true,
            },
            [503] = new TileData()
            {
                Type = 503,
                Name = "Stardust_Brick",
                Solid = true,
            },
            [504] = new TileData()
            {
                Type = 504,
                Name = "Mystic_Snake_Rope",
            },
            [505] = new TileData()
            {
                Type = 505,
                Name = "Gold_Fish_Bowl",
            },
            [506] = new TileData()
            {
                Type = 506,
                Name = "Bast_Statue",
            },
            [507] = new TileData()
            {
                Type = 507,
                Name = "Gold_Starry_Block",
                Solid = true,
            },
            [508] = new TileData()
            {
                Type = 508,
                Name = "Blue_Starry_Block",
                Solid = true,
            },
            [509] = new TileData()
            {
                Type = 509,
                Name = "Void_Monolith",
            },
            [510] = new TileData()
            {
                Type = 510,
                Name = "Arrow_Sign",
            },
            [511] = new TileData()
            {
                Type = 511,
                Name = "Painted_Arrow_Sign",
            },
            [512] = new TileData()
            {
                Type = 512,
                Name = "Green_Moss_Brick",
                Solid = true,
            },
            [513] = new TileData()
            {
                Type = 513,
                Name = "Brown_Moss_Brick",
                Solid = true,
            },
            [514] = new TileData()
            {
                Type = 514,
                Name = "Red_Moss_Brick",
                Solid = true,
            },
            [515] = new TileData()
            {
                Type = 515,
                Name = "Blue_Moss_Brick",
                Solid = true,
            },
            [516] = new TileData()
            {
                Type = 516,
                Name = "Purple_Moss_Brick",
                Solid = true,
            },
            [517] = new TileData()
            {
                Type = 517,
                Name = "Lava_Moss_Brick",
                Solid = true,
            },
            [518] = new TileData()
            {
                Type = 518,
                Name = "Lily_Pads",
            },
            [519] = new TileData()
            {
                Type = 519,
                Name = "Cattails",
            },
            [520] = new TileData()
            {
                Type = 520,
                Name = "Plate",
            },
            [521] = new TileData()
            {
                Type = 521,
                Name = "Black_Dragonfly_Jar",
            },
            [522] = new TileData()
            {
                Type = 522,
                Name = "Blue_Dragonfly_Jar",
            },
            [523] = new TileData()
            {
                Type = 523,
                Name = "Green_Dragonfly_Jar",
            },
            [524] = new TileData()
            {
                Type = 524,
                Name = "Orange_Dragonfly_Jar",
            },
            [525] = new TileData()
            {
                Type = 525,
                Name = "Red_Dragonfly_Jar",
            },
            [526] = new TileData()
            {
                Type = 526,
                Name = "Yellow_Dragonfly_Jar",
            },
            [527] = new TileData()
            {
                Type = 527,
                Name = "Gold_Dragonfly_Jar",
            },
            [528] = new TileData()
            {
                Type = 528,
                Name = "Mushroom_Vines",
            },
            [529] = new TileData()
            {
                Type = 529,
                Name = "Sea_Oats",
                TextureGrid = new(16, 15),
            },
            [530] = new TileData()
            {
                Type = 530,
                Name = "Oasis_Plants",
            },
            [531] = new TileData()
            {
                Type = 531,
                Name = "Boulder_Statue",
            },
            [532] = new TileData()
            {
                Type = 532,
                Name = "Maggot_Cage",
            },
            [533] = new TileData()
            {
                Type = 533,
                Name = "Rat_Cage",
            },
            [534] = new TileData()
            {
                Type = 534,
                Name = "Krypton_Moss_Block",
                Solid = true,
            },
            [535] = new TileData()
            {
                Type = 535,
                Name = "Krypton_Moss_Brick",
                Solid = true,
            },
            [536] = new TileData()
            {
                Type = 536,
                Name = "Xenon_Moss_Block",
                Solid = true,
            },
            [537] = new TileData()
            {
                Type = 537,
                Name = "Xenon_Moss_Brick",
                Solid = true,
            },
            [538] = new TileData()
            {
                Type = 538,
                Name = "Ladybug_Cage",
            },
            [539] = new TileData()
            {
                Type = 539,
                Name = "Argon_Moss_Block",
                Solid = true,
            },
            [540] = new TileData()
            {
                Type = 540,
                Name = "Argon_Moss_Brick",
                Solid = true,
            },
            [541] = new TileData()
            {
                Type = 541,
                Name = "Echo_Block",
                Solid = true,
            },
            [542] = new TileData()
            {
                Type = 542,
                Name = "Owl_Cage",
            },
            [543] = new TileData()
            {
                Type = 543,
                Name = "Pupfish_Bowl",
            },
            [544] = new TileData()
            {
                Type = 544,
                Name = "Gold_Ladybug_Cage",
            },
            [545] = new TileData()
            {
                Type = 545,
                Name = "Lawn_Flamingo",
            },
            [546] = new TileData()
            {
                Type = 546,
                Name = "Grate_Block",
                Solid = true,
            },
            [547] = new TileData()
            {
                Type = 547,
                Name = "Potted_Trees",
            },
            [548] = new TileData()
            {
                Type = 548,
                Name = "Potted_Trees_(Group_2)",
            },
            [549] = new TileData()
            {
                Type = 549,
                Name = "Seaweed",
            },
            [550] = new TileData()
            {
                Type = 550,
                Name = "Turtle_Cage",
            },
            [551] = new TileData()
            {
                Type = 551,
                Name = "Jungle_Turtle_Cage",
            },
            [552] = new TileData()
            {
                Type = 552,
                Name = "Sandcastle_Bucket",
            },
            [553] = new TileData()
            {
                Type = 553,
                Name = "Grebe_Cage",
            },
            [554] = new TileData()
            {
                Type = 554,
                Name = "Seagull_Cage",
            },
            [555] = new TileData()
            {
                Type = 555,
                Name = "Water_Strider_Cage",
            },
            [556] = new TileData()
            {
                Type = 556,
                Name = "Gold_Water_Strider_Cage",
            },
            [557] = new TileData()
            {
                Type = 557,
                Name = "Grate_Block_closed",
                Solid = true,
            },
            [558] = new TileData()
            {
                Type = 558,
                Name = "Seahorse_Cage",
            },
            [559] = new TileData()
            {
                Type = 559,
                Name = "Gold_Seahorse_Cage",
            },
            [560] = new TileData()
            {
                Type = 560,
                Name = "Golf_Trophies",
            },
            [561] = new TileData()
            {
                Type = 561,
                Name = "Marble_Column",
            },
            [562] = new TileData()
            {
                Type = 562,
                Name = "Bamboo_Block",
                Solid = true,
            },
            [563] = new TileData()
            {
                Type = 563,
                Name = "Large_Bamboo_Block",
                Solid = true,
            },
            [564] = new TileData()
            {
                Type = 564,
                Name = "Plasma_Lamp",
            },
            [565] = new TileData()
            {
                Type = 565,
                Name = "Fog_Machine",
            },
            [566] = new TileData()
            {
                Type = 566,
                Name = "Amber_Stone_Block",
                Solid = true,
            },
            [567] = new TileData()
            {
                Type = 567,
                Name = "Garden_Gnome",
                TextureGrid = new(26, 18),
            },
            [568] = new TileData()
            {
                Type = 568,
                Name = "Pink_Fairy_Jar",
            },
            [569] = new TileData()
            {
                Type = 569,
                Name = "Green_Fairy_Jar",
            },
            [570] = new TileData()
            {
                Type = 570,
                Name = "Blue_Fairy_Jar",
            },
            [571] = new TileData()
            {
                Type = 571,
                Name = "Bamboo",
            },
            [572] = new TileData()
            {
                Type = 572,
                Name = "Souls_in_a_Bottle",
            },
            [573] = new TileData()
            {
                Type = 573,
                Name = "Tattered_Wood_Sign",
            },
            [574] = new TileData()
            {
                Type = 574,
                Name = "Boreal_Beam",
            },
            [575] = new TileData()
            {
                Type = 575,
                Name = "Rich_Mahogany_Beam",
            },
            [576] = new TileData()
            {
                Type = 576,
                Name = "Granite_Column",
            },
            [577] = new TileData()
            {
                Type = 577,
                Name = "Sandstone_Column",
            },
            [578] = new TileData()
            {
                Type = 578,
                Name = "Mushroom_Beam",
            },
            [579] = new TileData()
            {
                Type = 579,
                Name = "Rock_Golem_Head",
                TextureGrid = new(20, 20),
            },
            [580] = new TileData()
            {
                Type = 580,
                Name = "Hell_Butterfly_Jar",
            },
            [581] = new TileData()
            {
                Type = 581,
                Name = "Lavafly_in_a_Bottle",
            },
            [582] = new TileData()
            {
                Type = 582,
                Name = "Magma_Snail_Cage",
            },
            [583] = new TileData()
            {
                Type = 583,
                Name = "Topaz_Tree",
                TextureGrid = new(20, 20),
            },
            [584] = new TileData()
            {
                Type = 584,
                Name = "Amethyst_Tree",
                TextureGrid = new(20, 20),
            },
            [585] = new TileData()
            {
                Type = 585,
                Name = "Sapphire_Tree",
                TextureGrid = new(20, 20),
            },
            [586] = new TileData()
            {
                Type = 586,
                Name = "Emerald_Tree",
                TextureGrid = new(20, 20),
            },
            [587] = new TileData()
            {
                Type = 587,
                Name = "Ruby_Tree",
                TextureGrid = new(20, 20),
            },
            [588] = new TileData()
            {
                Type = 588,
                Name = "Diamond_Tree",
                TextureGrid = new(20, 20),
            },
            [589] = new TileData()
            {
                Type = 589,
                Name = "Amber_Tree",
                TextureGrid = new(20, 20),
            },
            [590] = new TileData()
            {
                Type = 590,
                Name = "Gem_Saplings",
            },
            [591] = new TileData()
            {
                Type = 591,
                Name = "Hanging_Pots",
            },
            [592] = new TileData()
            {
                Type = 592,
                Name = "Hanging_Brazier",
            },
            [593] = new TileData()
            {
                Type = 593,
                Name = "Mini_Volcano",
            },
            [594] = new TileData()
            {
                Type = 594,
                Name = "Large_Volcano",
            },
            [595] = new TileData()
            {
                Type = 595,
                Name = "Sakura_Sapling",
            },
            [596] = new TileData()
            {
                Type = 596,
                Name = "Vanity_Tree_Sakura",
                TextureGrid = new(20, 20),
            },
            [597] = new TileData()
            {
                Type = 597,
                Name = "Pylons",
            },
            [598] = new TileData()
            {
                Type = 598,
                Name = "Lava_Serpent_Bowl",
            },
            [599] = new TileData()
            {
                Type = 599,
                Name = "Amethyst_Bunny_Cage",
            },
            [600] = new TileData()
            {
                Type = 600,
                Name = "Topaz_Bunny_Cage",
            },
            [601] = new TileData()
            {
                Type = 601,
                Name = "Sapphire_Bunny_Cage",
            },
            [602] = new TileData()
            {
                Type = 602,
                Name = "Emerald_Bunny_Cage",
            },
            [603] = new TileData()
            {
                Type = 603,
                Name = "Ruby_Bunny_Cage",
            },
            [604] = new TileData()
            {
                Type = 604,
                Name = "Diamond_Bunny_Cage",
            },
            [605] = new TileData()
            {
                Type = 605,
                Name = "Amber_Bunny_Cage",
            },
            [606] = new TileData()
            {
                Type = 606,
                Name = "Amethyst_Squirrel_Cage",
            },
            [607] = new TileData()
            {
                Type = 607,
                Name = "Topaz_Squirrel_Cage",
            },
            [608] = new TileData()
            {
                Type = 608,
                Name = "Sapphire_Squirrel_Cage",
            },
            [609] = new TileData()
            {
                Type = 609,
                Name = "Emerald_Squirrel_Cage",
            },
            [610] = new TileData()
            {
                Type = 610,
                Name = "Ruby_Squirrel_Cage",
            },
            [611] = new TileData()
            {
                Type = 611,
                Name = "Diamond_Squirrel_Cage",
            },
            [612] = new TileData()
            {
                Type = 612,
                Name = "Amber_Squirrel_Cage",
            },
            [613] = new TileData()
            {
                Type = 613,
                Name = "Potted_Lava_Plants",
            },
            [614] = new TileData()
            {
                Type = 614,
                Name = "Potted_Ember_Tendrils",
            },
            [615] = new TileData()
            {
                Type = 615,
                Name = "Yellow_Willow_Sapling",
            },
            [616] = new TileData()
            {
                Type = 616,
                Name = "Vanity_Tree_Yellow_Willow",
                TextureGrid = new(20, 20),
            },
            [617] = new TileData()
            {
                Type = 617,
                Name = "Relic_Base",
            },
            [618] = new TileData()
            {
                Type = 618,
                Name = "Stone_Accent_Slab",
                Solid = true,
            },
            [619] = new TileData()
            {
                Type = 619,
                Name = "Truffle_Worm_Cage",
            },
            [620] = new TileData()
            {
                Type = 620,
                Name = "Prismatic_Lacewing_Jar",
            },
            [621] = new TileData()
            {
                Type = 621,
                Name = "Slice_of_Cake",
            },
            [622] = new TileData()
            {
                Type = 622,
                Name = "Teapot",
            },
            [623] = new TileData()
            {
                Type = 623,
                Name = "Potted_Crystal_Plants",
            },
            [624] = new TileData()
            {
                Type = 624,
                Name = "Abigail's_Flower",
                TextureGrid = new(20, 18),
            },
            [625] = new TileData()
            {
                Type = 625,
                Name = "Neon_Moss_Block",
                Solid = true,
            },
            [626] = new TileData()
            {
                Type = 626,
                Name = "Neon_Moss_Brick",
                Solid = true,
            },
            [627] = new TileData()
            {
                Type = 627,
                Name = "Helium_Moss_Block",
                Solid = true,
            },
            [628] = new TileData()
            {
                Type = 628,
                Name = "Helium_Moss_Brick",
                Solid = true,
            },
            [629] = new TileData()
            {
                Type = 629,
                Name = "Stinkbug_Cage",
            },
            [630] = new TileData()
            {
                Type = 630,
                Name = "Stinkbug_Blocker",
            },
            [631] = new TileData()
            {
                Type = 631,
                Name = "Ghostly_Stinkbug_Blocker",
            },
            [632] = new TileData()
            {
                Type = 632,
                Name = "Scarlet_Macaw_Cage",
            },
            [633] = new TileData()
            {
                Type = 633,
                Name = "Ash_Grass_Block",
                Solid = true,
            },
            [634] = new TileData()
            {
                Type = 634,
                Name = "Ash_Tree",
                TextureGrid = new(20, 20),
            },
            [635] = new TileData()
            {
                Type = 635,
                Name = "Ash_Wood",
                Solid = true,
            },
            [636] = new TileData()
            {
                Type = 636,
                Name = "Corrupt_Vines",
            },
            [637] = new TileData()
            {
                Type = 637,
                Name = "Ash_Short_Plants",
            },
            [638] = new TileData()
            {
                Type = 638,
                Name = "Ash_Vines",
            },
            [639] = new TileData()
            {
                Type = 639,
                Name = "Mana_Crystal",
            },
            [640] = new TileData()
            {
                Type = 640,
                Name = "Blue_Macaw_Cage",
            },
            [641] = new TileData()
            {
                Type = 641,
                Name = "Reef_Block",
                Solid = true,
            },
            [642] = new TileData()
            {
                Type = 642,
                Name = "Chlorophyte_Extractinator",
            },
            [643] = new TileData()
            {
                Type = 643,
                Name = "Toucan_Cage",
            },
            [644] = new TileData()
            {
                Type = 644,
                Name = "Yellow_Cockatiel_Cage",
            },
            [645] = new TileData()
            {
                Type = 645,
                Name = "Gray_Cockatiel_Cage",
            },
            [646] = new TileData()
            {
                Type = 646,
                Name = "Shadow_Candle",
                TextureGrid = new(16, 20),
            },
            [647] = new TileData()
            {
                Type = 647,
                Name = "Large_Rubble_(3x2)",
            },
            [648] = new TileData()
            {
                Type = 648,
                Name = "Large_Rubble_(3x2)_(Group_2)",
            },
            [649] = new TileData()
            {
                Type = 649,
                Name = "Medium_Rubble_(2x1)",
            },
            [650] = new TileData()
            {
                Type = 650,
                Name = "Small_Rubble_(1x1)",
            },
            [651] = new TileData()
            {
                Type = 651,
                Name = "Large_Rubble_(3x2)_(Group_3)",
            },
            [652] = new TileData()
            {
                Type = 652,
                Name = "Medium_Rubble_(2x2)",
            },
            [653] = new TileData()
            {
                Type = 653,
                Name = "Pots_(Rubblemaker)",
            },
            [654] = new TileData()
            {
                Type = 654,
                Name = "TNT_Barrel",
            },
            [655] = new TileData()
            {
                Type = 655,
                Name = "Plantera_Thorns",
            },
            [656] = new TileData()
            {
                Type = 656,
                Name = "Glow_Tulip",
            },
            [657] = new TileData()
            {
                Type = 657,
                Name = "Echo_Chamber",
            },
            [658] = new TileData()
            {
                Type = 658,
                Name = "Aether_Monolith",
            },
            [659] = new TileData()
            {
                Type = 659,
                Name = "Aetherium_Block",
                Solid = true,
            },
            [660] = new TileData()
            {
                Type = 660,
                Name = "Faeling_in_a_Bottle",
            },
            [661] = new TileData()
            {
                Type = 661,
                Name = "Corrupt_Jungle_Grass_Block",
                Solid = true,
            },
            [662] = new TileData()
            {
                Type = 662,
                Name = "Crimson_Jungle_Grass_Block",
                Solid = true,
            },
            [663] = new TileData()
            {
                Type = 663,
                Name = "Enchanted_Moondial",
            },
            [664] = new TileData()
            {
                Type = 664,
                Name = "Bouncy_Boulder",
                Solid = true,
            },
            [665] = new TileData()
            {
                Type = 665,
                Name = "Life_Crystal_Boulder",
            },
            [666] = new TileData()
            {
                Type = 666,
                Name = "Poo_Block",
                Solid = true,
            },
            [667] = new TileData()
            {
                Type = 667,
                Name = "Aetherium_Brick",
                Solid = true,
            },
            [668] = new TileData()
            {
                Type = 668,
                Name = "The_Dirtiest_Block",
                Solid = true,
            },
            [669] = new TileData()
            {
                Type = 669,
                Name = "Lunar_Rust_Brick",
                Solid = true,
            },
            [670] = new TileData()
            {
                Type = 670,
                Name = "Dark_Celestial_Brick",
                Solid = true,
            },
            [671] = new TileData()
            {
                Type = 671,
                Name = "Astra_Brick",
                Solid = true,
            },
            [672] = new TileData()
            {
                Type = 672,
                Name = "Cosmic_Ember_Brick",
                Solid = true,
            },
            [673] = new TileData()
            {
                Type = 673,
                Name = "Cryocore_Brick",
                Solid = true,
            },
            [674] = new TileData()
            {
                Type = 674,
                Name = "Mercury_Brick",
                Solid = true,
            },
            [675] = new TileData()
            {
                Type = 675,
                Name = "Star_Royale_Brick",
                Solid = true,
            },
            [676] = new TileData()
            {
                Type = 676,
                Name = "Heavenforge_Brick",
                Solid = true,
            },
            [677] = new TileData()
            {
                Type = 677,
                Name = "Ancient_Blue_Brick",
                Solid = true,
            },
            [678] = new TileData()
            {
                Type = 678,
                Name = "Ancient_Green_Brick",
                Solid = true,
            },
            [679] = new TileData()
            {
                Type = 679,
                Name = "Ancient_Pink_Brick",
                Solid = true,
            },
            [680] = new TileData()
            {
                Type = 680,
                Name = "Ancient_Gold_Brick",
                Solid = true,
            },
            [681] = new TileData()
            {
                Type = 681,
                Name = "Ancient_Silver_Brick",
                Solid = true,
            },
            [682] = new TileData()
            {
                Type = 682,
                Name = "Ancient_Copper_Brick",
                Solid = true,
            },
            [683] = new TileData()
            {
                Type = 683,
                Name = "Ancient_Obsidian_Brick",
                Solid = true,
            },
            [684] = new TileData()
            {
                Type = 684,
                Name = "Ancient_Hellstone_Brick",
                Solid = true,
            },
            [685] = new TileData()
            {
                Type = 685,
                Name = "Ancient_Cobalt_Brick",
                Solid = true,
            },
            [686] = new TileData()
            {
                Type = 686,
                Name = "Ancient_Mythril_Brick",
                Solid = true,
            },
            [687] = new TileData()
            {
                Type = 687,
                Name = "Lava_Moss_Brick",
                Solid = true,
            },
            [688] = new TileData()
            {
                Type = 688,
                Name = "Argon_Moss_Brick",
                Solid = true,
            },
            [689] = new TileData()
            {
                Type = 689,
                Name = "Krypton_Moss_Brick",
                Solid = true,
            },
            [690] = new TileData()
            {
                Type = 690,
                Name = "Xenon_Moss_Brick",
                Solid = true,
            },
            [691] = new TileData()
            {
                Type = 691,
                Name = "Neon_Moss_Brick",
                Solid = true,
            },
            [692] = new TileData()
            {
                Type = 692,
                Name = "Helium_Moss_Brick",
                Solid = true,
            },
        };
    }
}
